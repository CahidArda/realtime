...
import type { Realtime, Opts } from "./realtime.js"
import type { SystemEvent, UserEvent, RealtimeMessage } from "../types.js"

export interface BackendClientOpts<T> {
  channel?: string
  events?: Partial<{
...
export interface StreamResponseOpts<T> {
  channel?: string
  ...
  events?: Partial<{
    [N in keyof T]: Partial<{
      [K in keyof T[N]]: {
        resume?: boolean
        transform?: (event: { data: any; __event_path: string[]; __stream_id: string }) => string
      }
    }>
  }>
}
